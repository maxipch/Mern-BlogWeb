                ...
                if(err){
                    ...
                    if(!userStored.active){
                        ...
                        res.status(200).send({
                            ...
                            refreshToken: jwt.createAccessToken(userStored)
                        });
                    }
                }
            });
...
function getUsersActive(req, res){
    ...
    User.find({ active: query.active }).then(users => {     //Buscaremos los usuarios con la propiedad
        if(!users){                                         //active, unicamente aquellos que esten activados.
            res.status(404).send({ message: "No se ha encontrado ningun usuario"});
        } else {
            res.status(200).send({ users });
        }
            ...
            if(!userData){
                ...
                if(req.files){
                    ...
                    if(fileExt !== "png" && fileExt !== "jpg"){
                        res.status(400).send({message: "Error, el formato debe ser png o jpg"});
                    } else {
